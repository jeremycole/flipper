...
sub _after_new {
	my $self = shift;
	my $rh_params = shift;
	$self->{readonly} = 1; # sensible default - says the metadata module doesn't support writing
	return;
}
	...
	if (defined $masterpair) {
		if (!defined $self->{_cache}->{$masterpair}->{masterpair_info}) {
			...
			if ($masterpair eq 'mike') {
				...
				$self->{_cache}->{$masterpair}->{masterpair_info} = {	'read_ip'	=> '10.6.0.66',
											'write_ip'	=> '10.6.0.67',
	...
	if (defined $masterpair && defined $node) {
		if (!defined $self->{_cache}->{$masterpair}->{nodes_info}->{$node}) {
			...
			if ($node eq 'first_node') {
				$self->{_cache}->{$masterpair}->{nodes_info}->{$node} = { 'ip' => '10.6.0.64' };
			}
			else {
				$self->{_cache}->{$masterpair}->{nodes_info}->{$node} = { 'ip' => '10.6.0.65' };
			}
		}
...
sub get_node_names {
	...
	if (defined $masterpair) {
		if (!defined $self->{_cache}->{$masterpair}->{node_names}) {
			...
			$self->{_cache}->{$masterpair}->{node_names} = ['first_node', 'second_node'];
		}
