...
sub _after_new {
	...
	$self->{DBH} = DBI->connect(	$rh_params->{dsn},
					$rh_params->{username},
					$rh_params->{password},
...
sub get_masterpair_info {
	...
	if (defined $masterpair) {
		if (!defined $self->{_cache}->{$masterpair}->{masterpair_info}) {
			my $rah_masterpair_info = $self->{DBH}->selectall_arrayref(	'SELECT name,value FROM masterpair WHERE masterpair=?',
											{ Slice => {} },
											$masterpair
										);
...
sub get_node_info {
	...
	if (defined $masterpair && defined $node) {
		if (!defined $self->{_cache}->{$masterpair}->{nodes_info}->{$node}) {
			my $rah_node_info = $self->{DBH}->selectall_arrayref(	'SELECT name,value FROM node WHERE masterpair=? AND node=?',
										{ Slice => {} },
										$masterpair,
										$node,
									);
...
sub get_node_names {
	...
	if (defined $masterpair) {
		if (!defined $self->{_cache}->{$masterpair}->{node_names}) {
			my $rah_nodes = $self->{DBH}->selectall_arrayref(	'SELECT DISTINCT(node) FROM node WHERE masterpair=?',
										{ Slice => {} },
										$masterpair,
									);
...
sub get_masterpair_names {
	...
	if (!defined $self->{_cache}->{_masterpairs}) {
		my $rah_masterpairs = $self->{DBH}->selectall_arrayref(		'SELECT DISTINCT(masterpair) FROM masterpair',
										{ Slice => {} },
									);
